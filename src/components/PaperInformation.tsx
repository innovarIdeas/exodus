...
const PaperInformation = () => {
  ...
  const [form3, setForm3] = useState(false);  const contextValues = useContext(ContextStore);
  ...
  return (
    <div>
      ...
      <div className="w-[90%] lg:w-[70%] grid grid-cols-3 shadow-md bg-white text-blue-600">
        <p onClick={()=>handlePaperType()} className={`pl-4 border w-full h-full cursor-pointer py-4 ${form1 ? "text-white bg-main" : ""} `}>Paper Type</p>
        <p onClick={()=>handleBinding()} className={`${form2 ? "text-white bg-main" : ""} cursor-pointer pl-4 border w-full h-full py-4`}> Binding</p>
        <p onClick={()=>handlePrintType()} className={`${form3 ? "text-white bg-main" : ""} cursor-pointer pl-4 border w-full h-full py-4`}>Print Type</p>
      </div>
